...
function filtrarEmpresasAdmin(termo='') {
  ...
  document.getElementById('item-list').innerHTML=lista.map(e=>`
    <div class="item-card" onclick="abrirEmpresaAdmin('${e.id_empresa}')">
      ...
      <div class="item-sub"><span class="pill ${e.ativo?'on':'off'}">${e.ativo?'ATIVA':'INATIVA'}</span><span>${e.codigo} · ${e.total_usuarios} usuário${Number(e.total_usuarios)!==1?'s':''}</span></div>
...
async function salvarEmpresaExistente(id) {
  ...
  const res=await apiPost('rpc/atualizar_empresa_admin',{p_id_empresa:id,p_nome:nome,p_ativo:ativo});
...
async function salvarNovaEmpresa() {
  ...
  const res=await apiPost('rpc/criar_empresa_com_admin',{p_codigo:codigo,p_nome:nome,p_usuario_nome:usuarioNome,p_username:username,p_senha:senha});
