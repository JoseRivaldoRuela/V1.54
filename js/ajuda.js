...
async function abrirAreaChamados(modulo=ajudaModuloAtual){
  ...
  <div class="section-label"><span>Meus chamados / chamados da empresa</span></div>${lista.length?lista.map(c=>`<div style="background:var(--surface);border:1px solid var(--border);border-radius:10px;padding:14px;margin-bottom:9px;"><div style="display:flex;justify-content:space-between;gap:10px;flex-wrap:wrap;"><strong>#${c.id_chamado} - ${ajudaEsc(c.assunto)}</strong><span class="pill ${c.status==='RESOLVIDO'?'on':c.status==='CANCELADO'?'off':'warn'}">${ajudaEsc(c.status)}</span></div><div style="color:var(--text2);font-size:11px;margin:6px 0;">${ajudaEsc(c.modulo)} | ${ajudaEsc(c.prioridade)} | ${new Date(c.criado_em).toLocaleString('pt-BR')}</div><div style="font-size:13px;line-height:1.5;white-space:pre-wrap;">${ajudaEsc(c.descricao)}</div>${c.resposta?`<div style="margin-top:10px;padding:10px;background:var(--surface2);border-radius:8px;"><strong>Resposta do atendimento</strong><div style="margin-top:5px;white-space:pre-wrap;">${ajudaEsc(c.resposta)}</div></div>`:''}${isAdmin?`<div style="margin-top:10px;text-align:right;"><button class="btn btn-secondary" style="padding:6px 10px;font-size:11px;" onclick="atenderChamado(${c.id_chamado},'${ajudaEsc(c.status)}')">Atualizar atendimento</button></div>`:''}</div>`).join(''):'<div style="color:var(--text3);padding:18px;text-align:center;">Nenhum chamado registrado.</div>'}</div>`;
}
...
async function salvarChamado(){
  ...
  toast('Chamado registrado com sucesso.','success');await abrirAreaChamados(document.getElementById('ch-modulo').value);
}

async function atenderChamado(id,statusAtual){
  if(!isAdmin){toast('Apenas administradores podem atualizar o atendimento.','error');return;}
