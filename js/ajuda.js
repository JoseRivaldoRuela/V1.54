...
function renderMenuAjuda(filtro='') {
  ...
  const atendimentoTitulo=isSuperAdminJr?'Gerenciar chamados':'Atendimento';
  const atendimentoResumo=isSuperAdminJr?'Visualizar e responder todos os chamados':'Abrir e acompanhar chamados';
  document.getElementById('item-list').innerHTML=`<div class="item-card" onclick="abrirAreaChamados()" style="border-color:rgba(0,229,160,.35);"><div class="item-name">${atendimentoTitulo}</div><div class="item-sub">${atendimentoResumo}</div></div>${mods.map(m=>`<div class="item-card ${m.id===ajudaModuloAtual?'active':''}" onclick="selecionarModuloAjuda('${m.id}')"><div class="item-name">${m.icone} - ${m.titulo}</div><div class="item-sub">${m.resumo}</div></div>`).join('')||'<div class="empty-state" style="height:auto;padding:24px 8px;">Nenhum assunto encontrado.</div>'}`;
}

function selecionarModuloAjuda(id){
  ajudaModuloAtual=id;
  renderMenuAjuda(document.getElementById('search-input').value);
  renderConteudoAjuda();
  closeSidebar();
}
function navegarAjudaMobile(valor){
  if(valor==='atendimento') abrirAreaChamados();
  else selecionarModuloAjuda(valor);
}
...
function renderConteudoAjuda(){
  ...
  body.innerHTML=`<div style="max-width:900px;margin:auto;">
    <label class="help-mobile-nav"><span class="form-label">Assunto do help</span><select class="form-input form-select" onchange="navegarAjudaMobile(this.value)"><option value="atendimento">${isSuperAdminJr?'Gerenciar chamados':'Atendimento'}</option>${AJUDA_MODULOS.map(item=>`<option value="${item.id}" ${item.id===m.id?'selected':''}>${item.titulo}</option>`).join('')}</select></label>
    <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:16px;margin-bottom:20px;flex-wrap:wrap;"><div><div style="font-size:11px;color:var(--accent);font-family:var(--mono);text-transform:uppercase;">Central de ajuda / ${m.titulo}</div><h2 style="margin:6px 0;font-size:24px;">${m.titulo}</h2><p style="color:var(--text2);margin:0;">${m.resumo}</p></div><button class="btn btn-primary" onclick="abrirAreaChamados('${m.id}')">${isSuperAdminJr?'Gerenciar chamados':'Abrir chamado'}</button></div>
